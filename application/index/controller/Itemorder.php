...
class Itemorder extends Acl {
        ...
        if(isset($input['id'])){
            ...
            if(empty($input['id'])){
                ...
                if($vali===true){
                    ...
                    push_log('新しいサービスリスト[ '.$create_info['number'].' ]');//日志
                ...
                if($vali===true){
                    ...
                    push_log('サービスフォームを更新します[ '.$update_info['number'].' ]');//日志
            ...
            foreach ($arr as $arr_vo) {
                ...
                if(empty($class['type']['nod'])){
                    ...
                    if (!empty($class['money'])){
                        //新增对账单
                        $bill=Itemorderbill::create(['pid'=>$arr_vo,'account'=>$class['account'],'money'=>$class['money'],'data'=>'システムは自動的に生成されます','user'=>Session('is_user_id'),'time'=>time()]);
                    ...
                    push_log(($auto?'自動':'').'サービスフォームを確認します[ '.$class['number'].' ]');
                }else{
                    //反审核操作
                    //操作核销信息
                    if (!empty($class['money'])){
                    ...
                    push_log ('反レビューサービスフォーム[ '.$class['number'].' ]');
                }
            }
            $resule=['state'=>'success'];
        }
        ...
        if(isset_full($input,'arr') && is_array($input['arr'])){
            ...
            if(empty($data)){
                foreach ($class as $class_vo) {
                    push_log('サービスフォームを削除します[ '.$class_vo['number'].' ]');//日志
                ...
                $resule=['state'=>'error','info'=>'サービスオーダー[ '.$data[0]['number'].' ]レビュー、削除できません!'];
            }
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
    ...
    public function exports(){
        ...
        if(isset($input['mode'])){
            ...
            if(empty($input['mode'])){
                ...
                $excel=[];//初始化导出数据
                //1.填充标题数据
                array_push($excel,['type'=>'title','info'=>'サービスのリスト']);
                //2.构造表格数据
                $table_cell=[];//初始化表头数据
                //构造表头数据
                ...
                file_to_zip('シングル詳細サービス',$files);//打包输出数据
            }
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
    ...
    public function add_bill(){
        ...
        if(isset_full($input,'pid') && isset_full($input,'account') && isset_full($input,'money')){
            ...
            if($plus>$class['actual']){
                ...
                //4.返回数据
                push_log('サービス検証リスト情報を追加します[ '.$class['number'].' ]');//日志
    ...
    public function del_bill(){
        ...
        if(isset_full($input,'id')){
            ...
            //4.返回数据
            push_log('サービス取り消し伝票情報を削除する[ '.$class['number'].' ]');//日志
            $resule=['state'=>'success'];
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
    ...
    public function force_bill(){
        ...
        if(isset_full($input,'id')){
            ...
            //2.返回数据
            push_log('強制的にサービス取り消し伝票を処理する[ '.$class['number'].' ]');//日志
            $resule=['state'=>'success'];
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
    ...
    public function set_bills(){
        ...
        if(isset_full($input,'arr') && isset_full($input,'account') && isset_full($input,'money')){
            $number_arr=[];//初始化单据号数组
            $tip='サービス伝票の一括取り消し[ '.date('YmdHi',time()).' ]';
            ...
            if(!empty($number_arr)){
                push_log($tip.' - 一括取り消し総額为[ '.$input['money'].' ] - 实际核销单据号为[ '.implode(' | ',$number_arr).' ]');//日志
            }
    ...
    public function bill_export(){
        ...
        $excel=[];//初始化导出数据
        //1.填充标题数据
        array_push($excel,['type'=>'title','info'=>'サービス取り消し伝票情報']);
        //2.构造表格数据
        $table_cell=[];//初始化表头数据
        //构造表头数据
        foreach ($formfield as $formfield_vo) {
        ...
        //4.导出execl
        push_log('サービス取り消し伝票情報をエクスポートする');//日志
        export_excel('サービス取り消し伝票情報',$excel);
    }
}