...
class Orpurchase extends Acl {
    ...
    public function info(){
        ...
        if(isset_full($input,'id')){
            ...
            foreach ($info as $info_key=>$info_vo){
                ...
                //根据辅助属性重新赋值价格信息
                if(!empty($info_vo['attr']['nod']) && $info_vo['attr']['name']!='補助属性損失'){
    ...
    public function set(){
        ...
        if(isset_full($input,'oid')){
            ...
            if($vali===true){
                ...
                push_log('調達リストを追加しました[ '.$create_info['number'].' ]');//日志
    ...
    public function exports(){
        $input=input('get.');
        if(isset($input['mode'])){
            push_log('購入と図書館の詳細のエクスポートからのデータ');//日志
            $sql=get_sql($input,[
                'name'=>'continue',
                'number'=>'full_like',
                'storage'=>'full_dec_1',
            ...
            if(empty($input['mode'])){
                ...
                $excel=[];//初始化导出数据
                //1.填充标题数据
                array_push($excel,['type'=>'title','info'=>'購入リストの詳細リスト']);
                //2.构造表格数据
                $table_cell=[];//初始化表头数据
                //构造表头数据
                ...
                foreach ($arr as $arr_vo) {
                    $excel=[];//初始化导出数据
                    //1.填充标题数据
                    array_push($excel,['type'=>'title','info'=>'購入の詳細リスト']);
                    //2.添加基础字段
                ...
                file_to_zip('購入と保存の詳細単一の詳細',$files);//打包输出数据
            }
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
