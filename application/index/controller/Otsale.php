...
class Otsale extends Acl {
        ...
        if(isset($input['id'])){
            ...
            if(empty($input['id'])){
                ...
                if($vali===true){
                    ...
                    push_log('他のアウトレットリストを追加しました[ '.$create_info['number'].' ]');//日志
                ...
                if($vali===true){
                    ...
                    push_log('他のアウトレットリストを更新します[ '.$update_info['number'].' ]');//日志
        ...
        if(empty($arr)){
                ...
                if(empty($class['type']['nod'])){
                    ...
                    foreach ($info as $info_key=>$info_vo) {
                        if(!empty($info_vo['serial'])){
                            ...
                            if(!empty($serial)){
                                $auto&&(push_log('他のアウトレットリストの自動レビュー[ '.$class['number'].' ]失敗,理由:第'.($info_key+1).'行シリアルコードの状態が正しくありません!'));//日志
                                return json(['state'=>'error','info'=>'レビュー-その他のアウトレットリスト[ '.$class['number'].' ]失敗,理由:第'.($info_key+1).'行シリアルコードの状態が正しくありません!']);
                                exit;
                            }
                        }
                    }
                    ...
                    foreach ($info as $info_key=>$info_vo) {
                        if(!empty($info_vo['serial'])){
                            ...
                            if(!empty($serial)){
                                return json(['state'=>'error','info'=>'反レビュー-その他のアウトレットリスト[ '.$class['number'].' ]第'.($info_key+1).'行シリアルコードの状態が正しくありません!']);
                                exit;
                            }
                        }
                    }
            ...
            foreach ($arr as $arr_vo) {
                ...
                if(empty($class['type']['nod'])){
                    ...
                    push_log(($auto?'自動':'').'他のアウトレットリストを確認してください[ '.$class['number'].' ]');
                }else{
                    //反审核操作
                    foreach ($info as $info_vo){
                    ...
                    push_log ('反レビューその他のアウトレットリスト[ '.$class['number'].' ]');
                }
            }
            $resule=['state'=>'success'];
        }
        ...
        if(isset_full($input,'arr') && is_array($input['arr'])){
            ...
            if(empty($data)){
                foreach ($class as $class_vo) {
                    push_log('他のアウトレットリストを削除します[ '.$class_vo['number'].' ]');//日志
                ...
                $resule=['state'=>'error','info'=>'その他のアウトレットリスト[ '.$data[0]['number'].' ]レビュー、削除できません!'];
            }
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
    ...
    public function exports(){
        $input=input('get.');
        if(isset($input['mode'])){
            push_log('他のアウトレットリストデータをエクスポートします');//日志
            $sql=get_sql($input,[
                'name'=>'continue',
                'number'=>'full_like',
                'pagetype'=>'full_dec_1',
            ...
            if(empty($input['mode'])){
                ...
                foreach ($arr as $arr_vo) {
                    $excel=[];//初始化导出数据
                    //1.填充标题数据
                    array_push($excel,['type'=>'title','info'=>'その他のアウトレットリスト']);
                    //2.添加基础字段
                ...
                file_to_zip('その他のアウト単一の詳細',$files);//打包输出数据
            }
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
