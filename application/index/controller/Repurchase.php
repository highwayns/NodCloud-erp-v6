...
class Repurchase extends Acl {
        ...
        if(isset($input['id'])){
            ...
            if(empty($input['id'])){
                ...
                if($vali===true){
                    ...
                    push_log('新しい購入と返品注文[ '.$create_info['number'].' ]');//日志
                ...
                if($vali===true){
                    ...
                    push_log('購入リストと返品リストを更新します[ '.$update_info['number'].' ]');//日志
                ...
                if(empty($class['type']['nod'])){
                    ...
                    foreach ($info as $info_key=>$info_vo) {
                        if(!empty($info_vo['serial'])){
                            ...
                            if(!empty($serial)){
                                $auto&&(push_log('購入リストと返品リストを自動的に確認します[ '.$class['number'].' ]失敗,理由:第'.($info_key+1).'文字列コードのステータスが正しくありません!'));//日志
                                return json(['state'=>'error','info'=>'レビュー-注文の購入と返品[ '.$class['number'].' ]失敗,理由:第'.($info_key+1).'文字列コードのステータスが正しくありません!']);
                                exit;
                            }
                        }
                    }
                    ...
                    foreach ($info as $info_key=>$info_vo) {
                        if(!empty($info_vo['serial'])){
                            ...
                            if(!empty($serial)){
                                return json(['state'=>'error','info'=>'反レビュー-注文の購入と返品[ '.$class['number'].' ]第'.($info_key+1).'文字列コードのステータスが正しくありません!']);
                                exit;
                            }
                        }
                    }
            ...
            foreach ($arr as $arr_vo) {
                ...
                if(empty($class['type']['nod'])){
                    ...
                    if (!empty($class['money'])){
                        //新增对账单
                        $bill=Repurchasebill::create(['pid'=>$arr_vo,'account'=>$class['account'],'money'=>$class['money'],'data'=>'システムは自動的に生成されます','user'=>Session('is_user_id'),'time'=>time()]);
                    ...
                    push_log(($auto?'自動':'').'購入と返品注文を確認します[ '.$class['number'].' ]');
                }else{
                    //反审核操作
                    foreach ($info as $info_vo){
                    ...
                    push_log ('反レビューの購入と返品注文[ '.$class['number'].' ]');
                }
            }
            $resule=['state'=>'success'];
        }
        ...
        if(isset_full($input,'arr') && is_array($input['arr'])){
            ...
            if(empty($data)){
                foreach ($class as $class_vo) {
                    push_log('購入リストと返品リストを削除します[ '.$class_vo['number'].' ]');//日志
                ...
                $resule=['state'=>'error','info'=>'注文の購入と返品[ '.$data[0]['number'].' ]レビュー、削除できません!'];
            }
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
    ...
    public function exports(){
        $input=input('get.');
        if(isset($input['mode'])){
            push_log('商品の購入と返品リストデータをエクスポートします');//日志
            $sql=get_sql($input,[
                'name'=>'continue',
                'number'=>'full_like',
                'supplier'=>'full_division_in',
            ...
            if(empty($input['mode'])){
                ...
                $excel=[];//初始化导出数据
                //1.填充标题数据
                array_push($excel,['type'=>'title','info'=>'購入および返品リストのリスト']);
                //2.构造表格数据
                $table_cell=[];//初始化表头数据
                //构造表头数据
                ...
                //4.导出execl
                export_excel('購入および返品リストのリスト',$excel);
            }else{
                //详细报表
                $files=[];//初始化文件列表
                ...
                file_to_zip('注文の購入と返品',$files);//打包输出数据
            }
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
    ...
    public function add_bill(){
        ...
        if(isset_full($input,'pid') && isset_full($input,'account') && isset_full($input,'money')){
            ...
            if($plus>$class['actual']){
                ...
                //4.返回数据
                push_log('仕入れ返品取り消し伝票情報を追加する[ '.$class['number'].' ]');//日志
    ...
    public function del_bill(){
        ...
        if(isset_full($input,'id')){
            ...
            //4.返回数据
            push_log('仕入れ返品取り消し伝票情報を削除する[ '.$class['number'].' ]');//日志
            $resule=['state'=>'success'];
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
    ...
    public function force_bill(){
        ...
        if(isset_full($input,'id')){
            ...
            //2.返回数据
            push_log('仕入れ返品取り消し伝票を強制的に処理する[ '.$class['number'].' ]');//日志
            $resule=['state'=>'success'];
        }else{
            $resule=['state'=>'error','info'=>'入力されたパラメーターが不完全です!'];
        }
    ...
    public function set_bills(){
        ...
        if(isset_full($input,'arr') && isset_full($input,'account') && isset_full($input,'money')){
            $number_arr=[];//初始化单据号数组
            $tip='仕入れ返品伝票を一括で取り消す[ '.date('YmdHi',time()).' ]';
            ...
            if(!empty($number_arr)){
                push_log($tip.' - 一括取り消しの総額は[ '.$input['money'].' ] - 実際の取り消し伝票番号は[ '.implode(' | ',$number_arr).' ]');//日志
            }
    ...
    public function bill_export(){
        ...
        //4.导出execl
        push_log('仕入れ返品取り消し伝票情報をエクスポートする');//日志
        export_excel('仕入れ返品取り消し伝票情報',$excel);
    }
}