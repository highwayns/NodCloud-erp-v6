        ...
        if(cellname=="towarehouse"){
            for (var i = 0; i < warehouse_arr.db.length; i++) {
                if(warehouse_arr.db[i].id==value){
                    $(grid_id).jqGrid('setCell',rowid,'towarehouse_id',warehouse_arr.db[i].id);//设置调拨仓库ID
                    break;
                }
            }
        }
...
function pop_data(arr,type){
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        var row={
            ...
            towarehouse_id:type?'0':arr[i].towarehouse,
            ...
            towarehouse:type?'点击选择':arr[i].towarehouseinfo.name,
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if(data[i].set_id !== '-1'){
            ...
            if(!reg_test('plus',data[i].nums) || data[i].nums=='0'){
                ...
                return false;
            }else if(data[i].towarehouse_id=='0'){
                dump('第'+(i+1)+'行未选择调拨仓库,请核实!');
                return false;
            }else if((data[i].warehouse_id-0)==(data[i].towarehouse_id-0)){
                dump('第'+(i+1)+'行调拨仓库与现仓库相同,请核实!');
                return false;
            }
        }
    }
